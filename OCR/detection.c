	...
	for(int i=0;i<50;i++){
		...
		while(c[j]!=-1){
				if(c[j]==-2){break;}
					...
					int width1= get_width(c[j]
                                        ,a[i],width,image);
					int height2 = 
                                        get_height(a[i],height,image);
					SDL_Surface* new_surface=
                                        SDL_CreateRGBSurface(0,width1-1,
                                        height2-1,32,0,0,0,0);
					SDL_Rect first;
					first.x=c[j];
					first.y=a[i];
					first.w=width1;
					first.h=height2;
					SDL_BlitSurface(image,&first,
                                        new_surface,NULL);
					char Output[30];
					sprintf (Output,"%s%d%s","file/",
                                         j,"_pic.bmp");
...
SDL_Surface* sdlnewchar(SDL_Surface* img,int minw,int maxw,int minh, int maxh)
{
	Uint8 r,g,b;
	Uint32 pixel;
	SDL_Surface *abc = SDL_CreateRGBSurface(0,maxw-minw,maxh - minh,32,0,
                                                                    0,0, 0);
